// Edit Subtask (Satellite) Modal - in-place editing from galaxy view
...
interface EditSubtaskDialogProps {
  open: boolean;
  subtaskId: string;
  initialData?: { name: string; description?: string; estimatedHours?: number; priorityStars?: number; satelliteType?: string };
  onClose: () => void;
  onSuccess?: () => void;
}
  ...
  useEffect(() => {
    if (open && subtaskId) {
      if (initialData) {
        ...
        setSatelliteType((initialData.satelliteType as SatelliteType) || 'notes');
        setLoading(false);
      } else {
        setLoading(true);
        ...
        supabase
          .from('subtasks')
          .select('id, name, description, estimated_hours, priority_stars, satellite_type')
          .eq('id', subtaskId)
          .single()
          .then(({ data, error }) => {
            if (!error && data) {
              ...
              setSatelliteType((data.satellite_type as SatelliteType) || 'notes');
            }
            setLoading(false);
          });
      }
    }
  }, [open, subtaskId, initialData]);
          ...
          <form onSubmit={handleSubmit} style={{ padding: '32px' }}>
            ...
            <div style={{ marginBottom: '24px' }}>
              <label style={{ display: 'block', fontSize: '14px', fontWeight: 600, color: '#00d9ff', marginBottom: '12px' }}>
                Satellite Type (Galactic view)
              </label>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' }}>
                {SATELLITE_TYPES.map((s) => {
                  const isSelected = satelliteType === s.type;
                  const isHovered = hoveredType === s.type;
                  return (
                    <button
                      key={s.type}
                      type="button"
                      onClick={() => setSatelliteType(s.type)}
                      onMouseEnter={() => setHoveredType(s.type)}
                      onMouseLeave={() => setHoveredType(null)}
                      style={{
                        padding: '10px',
                        background: isSelected ? `${s.color}22` : 'rgba(0, 0, 0, 0.3)',
                    ...
                    >
                      <div style={{ marginBottom: '4px', display: 'flex', justifyContent: 'center' }}>
                        <SatelliteIcon type={satelliteTypeToSpacecraft(s.type)} size="sm" />
                      </div>
                      {s.name}
                    </button>
                  );
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', paddingTop: '8px', borderTop: '1px solid rgba(0, 217, 255, 0.1)' }}>
              ...
              <button
                ...
                style={{
                  ...
                  fontWeight: 600,
                  fontFamily: 'Orbitron, sans-serif',
                  cursor: updateSubtask.isPending ? 'not-allowed' : 'pointer',
                  opacity: updateSubtask.isPending ? 0.6 : 1,
                }}
              >
                {updateSubtask.isPending ? 'Saving...' : 'Save'}
              </button>
            </div>
          </form>
        )}
